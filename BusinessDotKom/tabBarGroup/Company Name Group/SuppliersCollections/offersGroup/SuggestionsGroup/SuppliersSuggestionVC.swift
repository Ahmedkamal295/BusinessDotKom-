...
class SuppliersSuggestionVC: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    @IBAction func btnServicesVC(_ sender: Any) {
        let vc = storyboard?.instantiateViewController(withIdentifier: "SuggestionServicesVC") as? SuggestionServicesVC
        self.navigationController?.pushViewController(vc!, animated: false)
    }
 func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
     ...
     func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
         let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? TableCellSuppliers
 ...
 //        cell?.lblSupplierName.text = NameSupplier[indexPath.row]
    ...
    extension SuppliersSuggestionVC : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
        func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
            return 7
        }
        
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? CollectionCellSupplier
